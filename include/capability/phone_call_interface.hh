...
/**
 * @brief Phone call state
 * @see IPhoneCallListener::callStateChanged
 */
enum class PhoneCallState {
    IDLE, /**< Idle state */
    OUTGOING, /**< Call receiving state */
    INCOMING, /**< Calling state */
    ESTABLISHED /**< Phone connected state */
};
...
public:
    ...
    /**
     * @brief Report changes in the phone call state to the user.
     * @param[in] state phone call state
     */
...
public:
    ...
    /**
     * @brief Notify when call is arrived
     * @param[in] payload event's payload
     */
    virtual void callArrived(const std::string& payload) = 0;

    /**
     * @brief Notify when call is ended
     * @param[in] payload event's payload
     */
    virtual void callEnded(const std::string& payload) = 0;

    /**
     * @brief Notify when call is established
     * @param[in] payload event's payload
     */
